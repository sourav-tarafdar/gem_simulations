#define UNUSED(expr) do { (void)(expr); } while (0)
    ...
    int main(int argc, char * argv[]) {
      ...
      Float_t mesh =   0.1;    //Coordinate of drift electrode/cathode along Z  in cm
      Float_t pad = -0.1;    // Coordinate of readout pad/ anode along Z in cm
	...
	if (argv[5]) {
      ...
      */
      int nevents = 3 ; // Number of events.
      /*
	if (argv[6]) {
        nevents = atoi(argv[6]);
      ...
      */
      ...
      TString filename;
      filename = Form("OUTFILE/singlegem_gemout_%s.root", argv[1]);
      ...
      gem->Initialise("ANSYS_geo/ELIST_singgem_course.lis",		  "ANSYS_geo/NLIST_singgem_course.lis",		  "ANSYS_geo/MPLIST_singgem_course.lis",                  "ANSYS_geo/PRNSOL_singgem_course.lis", "mm");
      ...
      */
      ...
      if (plotField) {
        ...
        fieldView->SetVoltageRange(-500., -150.); // gem example
        ...
        fieldView2->SetArea(-pitch/2, -0.11, pitch/2 , 0.1);
        fieldView2->SetVoltageRange(-600., 0.);
        //TCanvas* cf = new TCanvas("cf", "", 600, 600);
	...
	drift->EnablePlotting(driftView);
      ...
      for(int i=0; i <nevents; i++) {
        ...
        for (int j= 0 ; j < nelend; j++) {
          ...
          ntotalend++       ;
            if(za1 < -1.0* (metal+ kapton/2.+0.04))e_anode++ ; // 0.04 cm added to make sure that we are not counting electrons stuck at bottom of GEM
